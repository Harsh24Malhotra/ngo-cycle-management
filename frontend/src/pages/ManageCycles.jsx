...
export default function ManageCycles() {
  ...
  const handleReturn = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5000/api/cycles/${id}/return`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchCycles();
    } catch (err) { console.error(err); }
  ...
  const handleAssignSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5000/api/cycles/${actionCycle._id}/assign`, 
  ...
  const handleUpdateStatusSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5000/api/cycles/${actionCycle._id}/update`, 
